...
describe("[Main]", () => {
  ...
  beforeEach(() => {
    ...
    main.setQuestion = () => {}; // * for fetch response null at jest
    main.render(screen);
  });
  ...
  describe("[기능]", () => {
    describe("시작 전", () => {
      ...
      describe("start 버튼을 클릭", () => {
        ...
        it("텍스트가 '초기화'로 변경된다.", () => {
          userEvent.click(getByTestId(screen, "start-button"));
          expect(getByTestId(screen, "start-button").innerHTML).toEqual(
            "초기화"
          );
        });
      });
    ...
    describe("시작 후", () => {
      ...
      describe("'초기화' 버튼 클릭", () => {
        ...
        it("버튼 텍스트가 '시작'으로 변경되고, 모든 상태값이 초기로 돌아간다.", () => {
          userEvent.click(getByTestId(screen, "start-button"));
          expect(getByTestId(screen, "remain-time").innerHTML).toEqual(
            "남은 시간: 0초"
          );
          expect(getByTestId(screen, "score").innerHTML).toEqual(`점수: 0점`);
          expect(getByTestId(screen, "question").innerHTML).toEqual(
            "문제 단어"
          );
